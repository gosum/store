...
func CreateStorage(ctx context.Context, name string) (*DB, error) {
	...
	sm := "CREATE TABLE " + tableName + " (key text PRIMARY KEY,value text);"
...
func (db *DB) ReadOnly(ctx context.Context, f func(context.Context, tkv.Transaction) error) error {
	...
	if err := f(ctx, &dbTx{r: tx}); err != nil {
		return err
	}
...
func (db *DB) ReadWrite(ctx context.Context, f func(context.Context, tkv.Transaction) error) error {
	...
	if err := f(ctx, &dbTx{w: tx, r: txr}); err != nil {
		return err
	}
...
func (tx *dbTx) BufferWrites(writes []tkv.Write) error {
	...
	for _, w := range writes {
		...
		if w.Value == "" {
			...
			m = fmt.Sprintf("INSERT INTO %s (key, value) VALUES ('%s', '%s') ON CONFLICT (key) DO UPDATE SET key = '%s', value = '%s'", tableName, w.Key, w.Value, w.Key, w.Value)
		}
		_, err := tx.w.Exec(m)
		if err != nil {
			tx.w.Rollback()
